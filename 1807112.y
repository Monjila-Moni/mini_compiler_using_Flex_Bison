...
%token ROOT INT FLOAT CHAR NUM VAR IF ELIF ELSE SWITCH CASE DEFAULT LB RB PRINT BREAK FOR SIN COS TAN ARRAY OE ISPRIME FIBSERIES FACTORS WHILE
%nonassoc IFX
%nonassoc ELIF
%nonassoc ELSE
%nonassoc SWITCH
...
statement:';' 
        ...
        |SWITCH '(' expression ')' LB BLOCK  RB    {tem=$3;}
        | IF '(' expression ')' LB statement RB %prec IFX {
								if($3)
								{
	...
	| IF '(' expression ')' LB statement RB ELSE LB statement RB {
								 	if($3)
									{
										printf("\nvalue of expression in IF: %d\n",$6);
									}
	...
	| IF '(' expression ')' LB IF '(' expression ')' LB statement RB ELSE LB statement RB statement RB ELSE LB statement RB %prec IFX {
								 	if($3)
									{
										if($8)
	...
	| IF '(' expression ')' LB statement RB ELIF '(' expression ')' LB statement RB ELSE LB statement RB {
								 	if($3)
									{
										printf("\nvalue of expression in IF: %d\n",$6);
									}
...
BLOCK   : C
        ...
        ;
C   : C ';' C
	| CASE NUM ':' expression ';' BREAK ';' { if($2==tem){printf("Case %d executed and the expression value is=%d\n",$2,$4);
        f=1;
        }
        }
	;
D   : DEFAULT ':' expression ';' BREAK ';' {if(f==0){printf("Default Case executed and the expression value=%d\n",$3);}}
	;
